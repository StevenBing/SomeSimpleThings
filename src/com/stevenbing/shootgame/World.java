	...
	public void bangAction() {
		for (int i = 0; i < bullets.length; i++) {
			...
			for (int j = 0; j < enemies.length; j++) {
				FlyingObject fo = enemies[j];
//				System.out.println(bullet + "\t" + fo);
				if (bullet.isLife() && fo.isLife() && bullet.hit(fo)) {
					bullet.goDead();
					fo.goDead();
					if (fo instanceof Enemy) {
						Enemy enemy = (Enemy) fo;
						score += enemy.getScore();
					} else if (fo instanceof Award) {
						Award award = (Award) fo;
						switch (award.getType()) {
							case Award.DOUBLE_FIER :
								hero.addDoubleFire();
								break;
							case Award.LIFE :
								hero.addLife();
					...
					if (f instanceof Award) {
						...
						switch (type) {
							...
							case Award.TRIPLE_FIRE:
								hero.addDoubleFire(Award.TRIPLE_FIRE);
								break;
							case Award.LIFE :
								hero.addLife();
								break;
